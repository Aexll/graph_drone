...
def get_mini_graph_gif(historique, targets, dist_threshold, size=1.5, skin="default", frames=100, save_path="results.gif", duration=0.08):
    images = []
...
def evolve_graph_gif(nodes, targets, dist_threshold, steps=10000):
    histories = []
    best_error = gx.cout_graph_p2(nodes, targets)
    best_nodes = nodes

    for i in range(steps):
        nodes = gx.safe_mutate_nodes(nodes, np.random.randint(0, len(nodes)), dist_threshold, max(0.1*(1-i/steps), 0.004))
        error = gx.cout_graph_p2(nodes, targets)
        if error < best_error:
            best_error = error
            best_nodes = nodes
            histories.append(best_nodes.copy())
    return histories
...
if __name__ == "__main__" and TEST_IMAGE:
    ...
    img_default = get_mini_graph_image_from_dict(dict_nodes_histories[best], targets, dist_threshold, size=1.5, skin="default")
    img_stick = get_mini_graph_image_from_dict(dict_nodes_histories[best], targets, dist_threshold, size=1.5, skin="stick")
    img_black = get_mini_graph_image_from_dict(dict_nodes_histories[best], targets, dist_threshold, size=1.5, skin="black")
    img_constellation = get_mini_graph_image_from_dict(dict_nodes_histories[best], targets, dist_threshold, size=1.5, skin="constellation")
...
if __name__ == "__main__" and TEST_GIF:
    ...
    # nodes_histories = gx.optimize_nodes_history(nodes, targets, dist_threshold, 0.1, 10000,False,True)
    nodes_histories = evolve_graph_gif(nodes, targets, dist_threshold, steps=1000)

    get_mini_graph_gif(nodes_histories, targets, dist_threshold, size=1.5, skin="default", frames=1000, save_path="results.gif", duration=0.08)
