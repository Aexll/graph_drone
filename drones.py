...
def run_combined_tables():
    root = tk.Tk()
    root.title("Drone Tables - ξ(i, j) and ω(i, j)")
    ...
    # ξ(i, j) Table
    xi_label = tk.Label(main_frame, text="ξ(i, j) Table", font=("Arial", 14, "bold"))
    xi_label.grid(row=0, column=0, columnspan=Drone.DRONE_COUNT+1, pady=(0, 10))
    
    xi_table_frame = tk.Frame(main_frame)
    xi_table_frame.grid(row=1, column=0, padx=(0, 20))
    
    # Create ξ table headers
    for j in range(Drone.DRONE_COUNT):
        header = tk.Label(xi_table_frame, text=f"j={j}", borderwidth=1, relief="solid", width=6)
        header.grid(row=0, column=j+1)
    for i in range(Drone.DRONE_COUNT):
        header = tk.Label(xi_table_frame, text=f"i={i}", borderwidth=1, relief="solid", width=6)
        header.grid(row=i+1, column=0)
    
    # Create ξ table cells
    xi_cells = []
    for i in range(Drone.DRONE_COUNT):
        row = []
        for j in range(Drone.DRONE_COUNT):
            lbl = tk.Label(xi_table_frame, text="", borderwidth=1, relief="solid", width=6)
            lbl.grid(row=i+1, column=j+1)
            row.append(lbl)
        xi_cells.append(row)
    
    # ω(i, j) Table
    omega_label = tk.Label(main_frame, text="ω(i, j) Table", font=("Arial", 14, "bold"))
    omega_label.grid(row=0, column=1, columnspan=Drone.DRONE_COUNT+1, pady=(0, 10))
    
    omega_table_frame = tk.Frame(main_frame)
    omega_table_frame.grid(row=1, column=1)
    
    # Create ω table headers
    for j in range(Drone.DRONE_COUNT):
        header = tk.Label(omega_table_frame, text=f"j={j}", borderwidth=1, relief="solid", width=6)
        header.grid(row=0, column=j+1)
    for i in range(Drone.DRONE_COUNT):
        header = tk.Label(omega_table_frame, text=f"i={i}", borderwidth=1, relief="solid", width=6)
        header.grid(row=i+1, column=0)
    
    # Create ω table cells
    omega_cells = []
    for i in range(Drone.DRONE_COUNT):
        row = []
        for j in range(Drone.DRONE_COUNT):
            lbl = tk.Label(omega_table_frame, text="", borderwidth=1, relief="solid", width=6)
            lbl.grid(row=i+1, column=j+1)
            row.append(lbl)
        omega_cells.append(row)
    ...
    def update_tables():
        ...
        for i in range(n):
            for j in range(n):
                value = ξ(i, j, n)
                xi_cells[i][j].config(text=str(value))
        
        # Update ω table
        for i in range(n):
            for j in range(n):
                value = ω(i, j, n)
                omega_cells[i][j].config(text=str(value) if value < inf else "∞")
        
        root.after(200, update_tables)  # update every 200 ms
    
    update_tables()
    root.mainloop()


def run_tabbed_tables():
    root = tk.Tk()
    root.title("Drone Tables")
    ...
    # Create ξ(i, j) Tab
    xi_frame = ttk.Frame(notebook)
    notebook.add(xi_frame, text="ξ(i, j) Table")
    
    xi_table_frame = tk.Frame(xi_frame)
    xi_table_frame.pack(padx=10, pady=10)
    ...
    # Create ω(i, j) Tab
    omega_frame = ttk.Frame(notebook)
    notebook.add(omega_frame, text="ω(i, j) Table")
    
    omega_table_frame = tk.Frame(omega_frame)
    omega_table_frame.pack(padx=10, pady=10)
