...
from learning import optimize_nodes, make_error_function, mutate_nodes, safe_mutate_nodes
...
def safe_mutate_nodes_callback(sender, app_data):
    global coordinates
    coordinates = safe_mutate_nodes(coordinates, dist_threshold=DIST_THRESHOLD, sigma=10)
    draw_graph(None, None)
    update_error(None, None)
...
def main():
    ...
    with dpg.window(label="Info", width=WINDOW_INFO_WH[0], height=WINDOW_INFO_WH[1], pos=[WINDOW_GRAPH_WH[0], 0]):
        ...
        dpg.add_button(label="Safe Mutate",callback=safe_mutate_nodes_callback)
