...
class GraphNetworkVisualizer:
    ...
    def create_interface(self, num_layers: int = 12, image_size: int = 100, layer_spacing: int = 200, vertical_spacing: int = 120, skin: str = "default"):
        ...
        # Create main window
        with dpg.window(label="Graph Network Visualizer", 
                       width=max_x, height=max_y, 
                       tag="main_window"):
            
            # Create canvas for drawing
            with dpg.drawlist(width=max_x, height=max_y, tag="canvas"):
                # Add all graph images
                for shape_key, pos in self.graph_positions.items():
                    x, y = pos
    ...
    def run(self, num_layers: int = 6, image_size: int = 60, layer_spacing: int = 200, vertical_spacing: int = 120, skin: str = "default"):
        """Run the visualizer"""
        dpg.create_viewport(title="Graph Network Visualizer", width=1200, height=800)
...
# Usage example:
def visualize_graph_network(shapes_dict, transition_history, get_image_func, 
                           targets, dist_threshold, num_layers=12, image_size=60, layer_spacing=200, vertical_spacing=120, skin="default"):
    """
    ...
    visualizer = GraphNetworkVisualizer(
        shapes_dict, transition_history, get_image_func, targets, dist_threshold
    )
    visualizer.run(num_layers, image_size, layer_spacing, vertical_spacing, skin)
